	...
	public SignPage() {
		...
		ActionListener al = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String account = actf.getText();
				String id = idtf.getText();
				String pw = pwtf.getText();
				String pw2 = pwchecktf.getText();

				if (!account.equals(actf.getText().trim())) {
					JOptionPane.showMessageDialog(mainFrame, "계좌번호에 공백이 포함되면 안됩니다.");
				} else if (!id.equals(idtf.getText().trim())) {
					JOptionPane.showMessageDialog(mainFrame, "아이디에 공백이 포함되면 안됩니다.");
				} else if (!pw.equals(pwtf.getText().trim())) {
